...
echo -e "${YELLOW}=== 🕵️ 检查最新稳定内核版本 (Kernel.org) ===${NC}"

CFG_FILE="customization-stable.cfg" # 【注意】这里指向稳定版 cfg
if [ ! -f "$CFG_FILE" ]; then
...
# 2. 从 Github 获取最新的稳定 Tag (排除 RC)
echo "🌐 正在查询 Github 最新稳定标签..."
...
if [ -z "$LATEST_TAG" ]; then
    echo -e "${RED}❌ 错误: 无法从 Github 获取最新稳定版本标签${NC}"
    exit 1
fi
